...
const QA_WISDOM = [
  ...
  { type: 'humor',  icon: '☕', text: 'QA Engineer: "Can I manually test coffee? Asking for a friend."' },
...
// ─── Matrix Characters ─────────────────────────────────────────────────────────
const MATRIX_CHARS = 'PASS FAIL NULL 404 ✓ ✗ BUG FIX TEST QA AUTO CI CD API NULL ERROR 200 500 ASSERT MOCK STUB SPY'.split(' ');
          ...
          <div
            ...
            style={{
              ...
              filter: reveal > 0.2
                ...
                : lightMode ? `drop-shadow(0 0 4px rgba(99,102,241,0.3))` : 'none',
              userSelect: 'none',
            }}
          >
            {bug.emoji}
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        ...
        <span style={{ color: typeColor, fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase' }}>
          {typeLabel}
        </span>
      </div>
      <p style={{ color: '#e2e8f0', fontSize: 12, lineHeight: 1.5, margin: 0, fontFamily: 'Inter, sans-serif' }}>
        {item.text}
      </p>
    </div>
  );
...
const QACursorEffect = ({ lightMode = false, disableTooltips = false }) => {
  ...
  const pickWisdom = useCallback(() => {
    let idx;
    do { idx = Math.floor(Math.random() * QA_WISDOM.length); } while (idx === lastIdx.current);
    lastIdx.current = idx;
  ...
  return (
    <>
      ...
      {!disableTooltips && (
        <WisdomTooltip item={tooltip} pos={tooltipPos} />
      )}
    </>
  );
};
