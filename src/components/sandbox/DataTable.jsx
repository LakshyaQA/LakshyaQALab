...
const DataTable = () => {
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex flex-col space-y-3 p-4" data-testid="table-loading">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="h-10 bg-slate-200 dark:bg-slate-700 rounded animate-pulse w-full"
            ></div>
          ))}
        </div>
      )
    ...
    if (error) {
      return (
        <div
          className="p-8 text-center text-rose-500 bg-rose-50 dark:bg-rose-950/20"
        ...
        >
          ...
          <button
            onClick={fetchData}
            className="mt-4 px-4 py-2 bg-rose-100 dark:bg-rose-900 text-rose-600 dark:text-rose-300 rounded hover:bg-rose-200 dark:hover:bg-rose-800 transition-colors"
            data-testid="table-retry-btn"
          >
            Retry Connection
          </button>
        </div>
        ...
        >
              ...
              {['id', 'name', 'role', 'status'].map(key => (
                ...
                >
                  <div className="flex items-center">
                    ...
                    {sortConfig.key === key && (
                      <svg
                        className={`w-3 h-3 ml-1 transition-transform ${sortConfig.direction === 'desc' ? 'rotate-180' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
          ...
          <tbody>
            {paginatedData.map(row => (
              <tr
                key={row.id}
                className="bg-white border-b dark:bg-slate-900 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors"
  ...
  return (
    ...
    >
      <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50">
        ...
        <span className="text-xs text-indigo-600 dark:text-indigo-400 font-mono bg-indigo-50 dark:bg-indigo-900/30 px-2 py-1 rounded">
          Component Lab
        </span>
      </div>
      ...
      {!loading && !error && (
        <div className="px-6 py-4 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between bg-white dark:bg-slate-900">
          <span className="text-sm text-gray-500 dark:text-gray-400">
            ...
            to{' '}
            <span className="font-semibold text-gray-900 dark:text-white" data-testid="page-end">
              {Math.min(currentPage * rowsPerPage, sortedData.length)}
            </span>{' '}
            of{' '}
            <span className="font-semibold text-gray-900 dark:text-white" data-testid="page-total">
              {sortedData.length}
            </span>{' '}
            Entries
          </span>
