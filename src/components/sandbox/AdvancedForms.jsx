    ...
    <div className="space-y-12" id="forms" data-testid="advanced-forms-section">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-visible relative">
        <div className="p-8 border-b border-gray-100 dark:border-slate-700">
          <h2 className="text-2xl font-black text-gray-900 dark:text-white uppercase tracking-tight">Custom Date Picker</h2>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Stress test calendar navigation and specific day selection in a non-native widget.</p>
        </div>
        
        <div className="p-8">
           <div className="max-w-xs relative overflow-visible">
              <label className="block text-xs font-black uppercase tracking-widest text-gray-400 dark:text-slate-500 mb-2">Select Target Date</label>
              <div 
              ...
              >
                <span className={selectedDate ? 'text-gray-900 dark:text-white font-bold' : 'text-gray-400'}>
              ...
              {showCalendar && (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    >
                      <svg className="w-4 h-4 text-gray-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                    </button>
                    <span className="font-bold text-sm text-gray-900 dark:text-white" data-testid="calendar-month-year">
                    ...
                    >
                      <svg className="w-4 h-4 text-gray-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </button>
                  </div>

                  <div className="grid grid-cols-7 gap-1 text-center mb-1">
                    {['S','M','T','W','T','F','S'].map(d => <div key={d} className="text-[10px] font-black text-gray-400 uppercase">{d}</div>)}
                  </div>

                  <div className="grid grid-cols-7 gap-1">
                    {[...Array(getFirstDayOfMonth(viewDate.getMonth(), viewDate.getFullYear()))].map((_, i) => <div key={`empty-${i}`} />)}
                    {[...Array(getDaysInMonth(viewDate.getMonth(), viewDate.getFullYear()))].map((_, i) => {
                      const day = i + 1;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-slate-700">
          <h2 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight mb-2">Smart Range Slider</h2>
          <p className="text-xs text-gray-500 mb-6">Test drag interaction and dynamic numerical verification.</p>
          
          <div className="bg-gray-50 dark:bg-slate-900/50 p-6 rounded-2xl">
            <div className="flex justify-between items-center mb-4">
               <span className="text-xs font-black uppercase tracking-widest text-gray-400">Budget Limit</span>
               <span className="text-lg font-black text-blue-600 px-3 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-xl" data-testid="slider-value">
                 ${priceRange}
               </span>
            </div>
            <input 
        ...
        {/* 3. MULTI-SELECT TAGS */}
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-slate-700">
          <h2 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight mb-2">Tagging Input</h2>
          <p className="text-xs text-gray-500 mb-6">Test keystroke interactions (Enter to add) and DOM element removal.</p>
          ...
          <div className="bg-gray-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-gray-100 dark:border-slate-700">
            <div className="flex flex-wrap gap-2 mb-3" data-testid="tag-container">
              {tags.map(tag => (
                <span key={tag} className="px-3 py-1.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-full text-xs font-bold text-gray-700 dark:text-slate-300 flex items-center gap-2 group">
                  {tag}
                  <button onClick={() => removeTag(tag)} className="text-gray-400 hover:text-rose-500 transition-colors" data-testid={`remove-tag-${tag.toLowerCase()}`}>
      ...
      <div className="p-6 bg-blue-600 rounded-3xl text-white flex flex-col md:flex-row items-center justify-between gap-4">
         <div className="flex items-center gap-3">
            ...
            <p className="text-xs font-medium leading-relaxed max-w-sm">
               Can your automation select **April 15, 2025** on the calendar? Try targeting by Month name navigation first.
            </p>
         </div>
