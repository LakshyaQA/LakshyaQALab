...
const XPathLab = () => {
  ...
  useEffect(() => {
    // Reset window and document scroll to top for maximum compatibility
    window.scrollTo(0, 0)
  ...
  const analyzeXpath = () => {
    ...
    if (openBrackets !== closeBrackets) {
      ...
      if (openBrackets > closeBrackets) {
        // Under-closed: Find last valid closed bracket or just append ']'
        const lastClosed = fixed.lastIndexOf(']')
        if (lastClosed !== -1) {
          fixed = fixed.substring(0, lastClosed + 1)
        } else if (fixed.includes('[')) {
          fixed += ']'
        }
      } else if (closeBrackets > openBrackets) {
        // Over-closed: Strip trailing ']' characters that have no owner
        fixed = fixed.replace(/\]+$/, '')
      }
    }
    ...
    // ── Step 8: Clean trailing junk and internal spaces ──
    fixed = fixed.replace(/\s+/g, '') // Remove all spaces for clean XPath
    ...
    // ── Step 10: Generate Professional Platform-Aware Alternatives ──
    const valMatch = fixed.match(/['"](.*?)['"]/)
    const tagMatch = fixed.match(/\/\/(\w+)/)
    const val = valMatch?.[1] || 'value'

    let best5 = []
    if (activeTool === 'AppiumAndroid') {
      best5 = [
        fixed,
        `//*[@resource-id='com.app:id/${val}']`,
        `//android.widget.TextView[@text='${val}']`,
        `//android.view.View[@content-desc='${val}']`,
        `//*[@resource-id='${val}'] // Simple ID fallback`,
      ]
    } else if (activeTool === 'AppiumIOS') {
      best5 = [
        fixed,
        `//*[@name='${val}']`,
        `//XCUIElementTypeStaticText[@label='${val}']`,
        `//XCUIElementTypeButton[@name='${val}']`,
        `//*[@accessibility-id='${val}']`,
      ]
    } else {
      // Standard Web (Selenium / Playwright)
  ...
  const askAi = async () => {
    ...
    try {
      ...
      addLog('error', `AI Service Error: ${err.message}`)

      // Professional Graceful Degradation:
      // If AI fails, automatically perform local analysis and show a warning
      analyzeXpath() // Trigger local fixer

      // We need to append the AI error message to the results generated by analyzeXpath
      setAnalysis(prev => ({
        ...prev,
        errors: [
          `⚠️ AI Engine Offline (${err.message}). Showing Local Smart Fix instead.`,
          'Action: Add your own OpenRouter.ai API Key in Settings ⚙️ for Expert AI analysis.',
          ...(prev?.errors || []),
        ],
        explanation:
          'The Expert AI analysis is currently unavailable. This result was generated by the local heuristic engine, which performs character stripping, bracket balancing, and platform-specific reconstruction.',
        proTip:
          'You can restore Expert AI analysis by providing a personal API key in the settings gear above.',
      }))
    } finally {
