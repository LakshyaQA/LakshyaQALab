...
const StatusPage = () => {
  ...
  const statusInfo = {
    '503': {
      ...
      message: "We're currently performing some scheduled maintenance to improve your playground experience. We'll be back online in a few minutes.",
      icon: (
    ...
    '500': {
      ...
      message: "Something went wrong on our end. Our QA team has been notified and is investigating the issue.",
      icon: (
        <svg className="w-20 h-20 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    '404': {
      ...
      message: "The page you're looking for doesn't exist or has been moved. Use this for testing broken links.",
      icon: (
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-slate-900 px-4 transition-colors duration-300">
      <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-3xl shadow-xl p-8 text-center border border-gray-100 dark:border-slate-700">
        ...
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2" data-testid="status-code">
          {statusCode}
        </h1>
        <h2 className="text-xl font-semibold text-gray-700 dark:text-slate-200 mb-4" data-testid="status-title">
          {info.title}
        </h2>
        <p className="text-gray-500 dark:text-gray-400 mb-8 leading-relaxed" data-testid="status-message">
          {info.message}
        </p>
